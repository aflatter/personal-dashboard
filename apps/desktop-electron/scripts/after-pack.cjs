...
// otherwise ESM/TS). Pure sanity assertion, no copying: the backend ships via
// the extraResources fileset rooted at .build/resources — one level above the
// backend, because builder's copy filter (app-builder-lib util/filter.js)
// hard-rejects a node_modules at a fileset's ROOT while allowing nested ones.
// If someone re-roots that fileset at the backend dir, the deps (incl. the
...
module.exports = async function afterPack(context) {
  const addon = path.join(
    ...
    "Contents/Resources/backend/node_modules/secretspec-darwin-arm64/secretspec.darwin-arm64.node",
  );
  if (!existsSync(addon)) throw new Error(`afterPack: native addon missing at ${addon}`);
  console.log("  • afterPack: backend node_modules shipped intact (secretspec .node present)");
};
