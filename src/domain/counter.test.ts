...
describe('counter', () => {
  ...
  it('classifies status by the thresholds (inclusive)', () => {
    expect(counter(NOW - 7 * DAY, THRESHOLDS, NOW).status).toBe('aktuell');
    expect(counter(NOW - 8 * DAY, THRESHOLDS, NOW).status).toBe('fällig bald');
    expect(counter(NOW - 21 * DAY, THRESHOLDS, NOW).status).toBe('fällig bald');
    expect(counter(NOW - 22 * DAY, THRESHOLDS, NOW).status).toBe('überfällig');
  });
